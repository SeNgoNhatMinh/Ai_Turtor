...
@Slf4j
@Service
public class CourseSearchContextLimitService {
    ...
    public CourseSearchContextLimitService(OpenRouterChatService chatService) {
        this.chatService = chatService;
    }

    public List<RetrievedCourseChunk> applyBudget(List<RetrievedCourseChunk> chunks) {
        ...
        List<RetrievedCourseChunk> selected = new ArrayList<>();
        int used = 0;
        for (RetrievedCourseChunk chunk : chunks) {
            ...
            selected.add(new RetrievedCourseChunk(
                    partial,
                    chunk.score(),
                    chunk.materialId(),
                    chunk.courseId(),
