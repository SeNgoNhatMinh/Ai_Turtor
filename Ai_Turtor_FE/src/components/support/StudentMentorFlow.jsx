...
function StudentMentorFlow({ escalation, currentUser, compact = false, onEscalationChange }) {
  ...
  const detailQuery = useQuery({
    ...
    queryFn: async ({ signal }) =>
      normalizeEscalationDetailResponse(await supportChatApi.getEscalationDetail(escalationId, { signal })),
  ...
  const mentors = useMemo(
    () =>
      offeredMentors.length ? offeredMentors : Array.isArray(detail?.suggestedMentors) ? detail.suggestedMentors : [],
  ...
  const selectedMentorOnline = selectedMentor?.online === true;
  ...
  const updateEscalationCache = useCallback(
    (next) => {
      if (!next?.id) return;
      queryClient.setQueryData(queryKeys.studentMentorRequestDetail(next.id), (current) => ({
        ...(current || {}),
        ...next,
      }));
      if (userId) {
        queryClient.setQueryData(queryKeys.studentMentorRequests(userId), (current = []) =>
  ...
  const offerMutation = useMutation({
    ...
    onError: (error) => setActionError(getUserFacingError(error, "Không thể tìm giáo viên phù hợp cho câu hỏi này.")),
  });

  const selectMutation = useMutation({
    mutationFn: () =>
      supportChatApi.selectMentor({
    ...
    onError: (error) => setActionError(getUserFacingError(error, "Không thể kết nối với giáo viên này.")),
  });
  ...
  useRealtimeEvent(REALTIME_EVENT_TYPES.teacherPresence, (event) => {
    const teacherId = String(event?.data?.teacherId || event?.entityId || "");
    if (!teacherId || !mentors.some((mentor) => String(mentor.id) === teacherId)) return;
    const online = event?.data?.online === true || event?.status === "ONLINE";
    setOfferedMentors((current) => {
      const source = current.length > 0 ? current : mentors;
      return source.map((mentor) => (String(mentor.id) === teacherId ? { ...mentor, online } : mentor));
    });
  });
  ...
  if (chatRoomId) {
    const isConversationClosed = status.includes("ANSWERED") || ["COMPLETED", "CLOSED", "CANCELLED"].includes(status);
    return (
      <SupportChatRoom
    ...
    <section className="mentor-selection-flow">
      <div className="mentor-selection-flow__heading">
        <div>
          ...
          <span>
            Hệ thống ưu tiên giáo viên phụ trách lớp. Bạn có thể gửi yêu cầu cho cả giáo viên đang online hoặc offline.
          </span>
        </div>
      ...
      {mentors.length === 0 ? (
        ...
        <>
          <Radio.Group
            value={effectiveMentorId}
            onChange={(event) => setSelectedMentorId(event.target.value)}
            className="mentor-selection-list"
          >
            {mentors.map((mentor) => (
              <Radio key={mentor.id} value={mentor.id} className="mentor-selection-option">
                ...
                <span className="mentor-selection-option__copy">
                  ...
                  <span className={`mentor-presence mentor-presence--${mentor.online === true ? "online" : "offline"}`}>
                    <i aria-hidden="true" />
                    {mentor.online === true ? "Online" : "Offline"}
                  </span>
                  <span>{mentor.matchReason || mentor.description || "Sẵn sàng hỗ trợ câu hỏi của môn học này."}</span>
                  <small>
                    {Number.isFinite(Number(mentor.averageRating)) && (
                      <>
                        <Star size={11} /> {Number(mentor.averageRating).toFixed(1)}
                      </>
                    )}
          ...
          {!selectedMentorOnline && selectedMentor && (
            <Alert
              type="info"
              showIcon
              title={`${selectedMentor.mentorName || "Giáo viên này"} đang offline. Yêu cầu vẫn sẽ được gửi đúng giảng viên và họ có thể phản hồi khi quay lại.`}
            />
          )}
          <ActionButton
            intent="primary"
            loading={selectMutation.isPending}
            disabled={!effectiveMentorId}
            onClick={chooseMentor}
          >
            {selectedMentorOnline ? "Bắt đầu trao đổi" : "Gửi yêu cầu đến giảng viên"}
          </ActionButton>
        </>
      )}
    </section>
