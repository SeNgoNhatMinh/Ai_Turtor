  ...
  it('requires Admin to analyze and select backend TOC items before importing', async () => {
    ...
    render(
      <ImportWebsiteModal
        ...
        syllabusDescription="Con trỏ: Khai báo và sử dụng con trỏ"
  ...
  it('selects and imports every TOC item without a fixed 50-item cap', async () => {
    ...
    render(
      <ImportWebsiteModal
        ...
        courseId="OOP"
        currentUser={{ id: 'admin-1' }}
        syllabusDescription="JVM: Cấu trúc và hoạt động"
        materialApi={materialApi}
        triggerToast={vi.fn()}
        onUploaded={vi.fn()}
        isAdmin
  ...
  it('adds multiple same-site extensionless paths to the import selection', async () => {
    ...
    render(
      <ImportWebsiteModal
        ...
        courseId="PRF192"
        currentUser={{ id: 'admin-1' }}
        syllabusDescription="Lập trình C: Các khái niệm nền tảng"
        materialApi={materialApi}
        triggerToast={vi.fn()}
        onUploaded={vi.fn()}
        isAdmin
