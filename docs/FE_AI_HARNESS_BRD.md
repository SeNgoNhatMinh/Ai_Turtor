...
13. `Done` - Pure harness response contract có unit tests cho RAG/CODE/ESCALATE, wrapper, deduplication, malformed response và failed business envelope.

### Live verification result

- `CODE`: pass qua student-chat webhook.
- `ESCALATE`: pass; escalation offer/select tạo được ChatRoom qua backend canonical API.
- Senior approve/reject: pass qua n8n.
- `RAG`: blocked do backend runtime lỗi embedding, không phải lỗi render/envelope FE.
- Teacher final answer: payload chạy backend-direct nhưng node n8n trả business failure; workflow cần sửa.
- Conversation rollover và pin persistence: chưa thể xác nhận vì chưa có persisted 10-turn conversation/message IDs từ các branch đang chạy.
- Chi tiết và bằng chứng không chứa credential được ghi tại `docs/FE_N8N_E2E_REPORT.md`.
- Tiếp tục giữ `VITE_N8N_QUIZ_ENABLED=false` cho tới khi `/quiz-generate` và `/quiz-submit` trả quiz session đầy đủ.
...
Status: completed. Deterministic FE contract tests run through `npm test`; live webhook verification is tracked separately.
...
- `npm test` pass cho response envelope, validation, chat/session, quiz và markdown security contracts.
